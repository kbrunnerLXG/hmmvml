...
for record in SeqIO.parse('/home/aditya/hmmvml/unreviewed/protein-matching-IPR000012.fasta', "fasta"):
    if "Vpr"  in str(record.description):
        ...
        continue
    protein_id.append(str(record.id).split("|")[0])
    proteins.append(str(record.seq))

df = pd.DataFrame({"Protein_id": protein_id,"Sequence":proteins,"Label":y})
print(df.head())
#
#     no_of_proteins += 1
